...
namespace CashierMgr
{
    ...
    public class BaseWebService : System.Web.Services.WebService
    {
        ...
        public BaseWebService()
        {
            ...
            //输入参数
            System.Collections.Specialized.NameValueCollection methodParameters = HttpContext.Current.Request.Form;
            string methodName = HttpContext.Current.Request.PathInfo.TrimStart(new char[] { '/' });
            Dictionary<string, string> paras = new Dictionary<string, string>();
            foreach (string name in methodParameters)
            {
                paras.Add(name, methodParameters[name]);
            }
            string path = ConfigurationManager.AppSettings["LogPath"].ToString();
            path = Server.MapPath(path);
            //将输入参数写到日志
            //this._log = new Logger(path, this.ToString(), methodName);
            //this._log.WriteXmlLog(paras);
        }
