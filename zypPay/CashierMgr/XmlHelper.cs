...
namespace CashierMgr
{
    public class XmlHelper
    {
        ...
        /// <returns></returns>
        public static T ReadXML<T>(string path, Type type)
        {
        ...
        /// <summary>
        /// 反序列化;DeserializeFromXml(xmlPath, typeof(Config)) as Config
        /// </summary>
        /// <param name="xmlFilePath">需要反序列化的XML文件的绝对路径</param>
        /// <param name="type">反序列化XML为哪种对象类型:typeof(Config)</param>
        /// <returns></returns>
        ...
        /// <summary>
        /// 序列化:SerializeToXml(config, config.GetType(), "d:\\config.xml", null);
        /// </summary>
        /// <param name="srcObject">对象的实例</param>
        /// <param name="type">对象类型</param>
        /// <param name="xmlFilePath">序列化之后的xml文件的绝对路径</param>
        /// <param name="xmlRootName">xml文件中根节点名称:当需要将多个对象实例序列化到同一个XML文件中的时候,
        /// xmlRootName就是所有对象共同的根节点名称,如果不指定,.net会默认给一个名称(ArrayOf+实体类名称)</param>
