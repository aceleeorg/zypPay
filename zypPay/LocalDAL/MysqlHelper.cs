...
namespace HYCashierDAL
{
    ...
    public sealed partial class MySqlHelper
    {
        ...
        public static MySqlCommand CurrentCommond { get; set; }

        public MySqlCommand DBCommond { get; set; }
        #region 实例方法
        ...
        {
            return ExecuteNonQuery2(ConnectionString,null, commandType, commandText, parms);
        }
        ...
        {
            return ExecuteScalar(ConnectionString, CommandType.Text, commandText, null);
        }
        ...
        private  void PrepareCommand2(MySqlCommand command, MySqlConnection connection, MySqlTransaction transaction, CommandType commandType, string commandText, MySqlParameter[] parms)
        {
            if (connection.State != ConnectionState.Open) connection.Open();
        ...
        private static int ExecuteNonQuery(MySqlConnection connection, MySqlTransaction transaction, CommandType commandType, string commandText, params MySqlParameter[] parms)
        {
            ...
            CurrentCommond = command;
            command.Parameters.Clear();
            return retval;
        }


        private int ExecuteNonQuery2(string connectionString, MySqlTransaction transaction, CommandType commandType, string commandText, params MySqlParameter[] parms)
        {
            using (MySqlConnection connection = new MySqlConnection(connectionString))
            {
                MySqlCommand command = new MySqlCommand();
                PrepareCommand2(command, connection, transaction, commandType, commandText, parms);
                int retval = command.ExecuteNonQuery();
                DBCommond = command;
                //command.Parameters.Clear();
                return retval;
