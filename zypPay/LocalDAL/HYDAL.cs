...
namespace LocalDAL
{
    public class HYDal
    {
        ...
        public bool InsertTradeInfo(string orderNo, string amount, string tradetype, string porderNo, string oprID, string channel, string clientID, string tid)
        {
            ...
            string sql = "insert into tradeinfo (localorderno,tradetime,amount,tradetype,oprid,Channel,clientID,TID,TradeStatus";
            if (!string.IsNullOrEmpty(porderNo))
            {
                sql += ",porderno) values ('" + orderNo + "','" + tradeTime + "'," + amount + ",'" + tradetype + "','" + oprID + "','" + channel + "','" + clientID + "','" + tid + "','NoPay','" + porderNo + "')";
            }
            else
            {
                sql += ") values  ('" + orderNo + "','" + tradeTime + "'," + amount + ",'" + tradetype + "','" + oprID + "','" + channel + "','" + clientID + "','" + tid + "','NoPay')";
            }
        ...
        public DataTable GetTodayTradeInfo(string oprID, string clientID, string channel, bool isAdmin)
        {
            DataTable dt = null;
            //if (DBHelper == null) DBHelper = new SQLiteHelper(DBSource);
        ...
        public DataTable GetTradeInfo(string startTime, string endTime, string status, string flowNo, string tradeType, string channel, string clientID, string oprID, int start, int end, bool isAdmin)
        {
            DataTable dt = null;
            //if (DBHelper == null) DBHelper = new SQLiteHelper(DBSource);
...
ServerFlowNo as 渠道流水号,case tradestatus when 'OK' then '成功' when 'NoPay' then '未支付' when 'OK_Refund' then '成功退款' when 'Fail' then '失败' else null end 状态, tradetype as 交易类型
        ...
        public DataTable GetAllTradeInfo(string startTime, string endTime, string status, string flowNo, string tradeType, string channel, string clientID, string oprID, bool isAdmin)
        {
            DataTable dt = null;
            // if (DBHelper == null) DBHelper = new SQLiteHelper(DBSource);
            string sql = @"select tradetime as 交易时间 ,amount as 金额, localorderno as 系统流水号,
...
tradetype as 交易类型,ErrorCode as 失败原因,CustomerName as 支付账号,OprID as 操作员 from tradeinfo where 1=1 ";
        ...
        public DataTable GetTradeStatistic(string startTime, string endTime, string status, string flowNo, string tradeType, string channel, string clientID, string oprID, bool isAdmin)
        {
            DataTable dt = null;
            // if (DBHelper == null) DBHelper = new SQLiteHelper(DBSource);
...
from tradeinfo where sendflag is null";
    ...
    public class HYDal_L
    {
        ...
        public HYDal_L(string connectionString)
        {
            DBHelper = new MySqlHelper(connectionString);
        }
